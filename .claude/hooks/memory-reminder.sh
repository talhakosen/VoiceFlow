...
PY_CHANGES=$(echo "$ALL_CHANGES" | grep -E '\.py$' || true)
if [ -n "$PY_CHANGES" ]; then
  PY_COUNT=$(echo "$PY_CHANGES" | wc -l | tr -d ' ')
  DOCS_FILE="$CLAUDE_PROJECT_DIR/docs/backend-architecture.md"
  if [ -f "$DOCS_FILE" ]; then
    ...
    if [ "$DOCS_AGE" -gt 3600 ] && [ "$PY_COUNT" -gt 2 ]; then
      REMINDERS="${REMINDERS}
- docs/backend-architecture.md: ${PY_COUNT} Python dosyası değişti — API/mimari güncel mi?"
    fi
  fi
fi
...
if [ -n "$SWIFT_CHANGES" ]; then
  ...
  DOCS_FILE="$CLAUDE_PROJECT_DIR/docs/app-architecture.md"
  if [ -f "$DOCS_FILE" ]; then
    DOCS_MOD=$(stat -f %m "$DOCS_FILE" 2>/dev/null || stat -c %Y "$DOCS_FILE" 2>/dev/null || echo "0")
    NOW=$(date +%s)
    DOCS_AGE=$(( NOW - DOCS_MOD ))
    if [ "$DOCS_AGE" -gt 3600 ] && [ "$SWIFT_COUNT" -gt 1 ]; then
      REMINDERS="${REMINDERS}
- docs/app-architecture.md: ${SWIFT_COUNT} Swift dosyası değişti — güncel mi?"
    fi
  fi
fi
...
if [ -f "$PLAN_FILE" ]; then
  ...
  if [ "$PLAN_AGE" -gt 7200 ] && [ -n "$PY_CHANGES" ]; then
    REMINDERS="${REMINDERS}
