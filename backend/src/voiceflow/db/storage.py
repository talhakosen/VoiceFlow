...
async def get_training_sentences(domain: str | None = None, limit: int = 50) -> list[dict]:
    """Return random training sentences, optionally filtered by domain."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        if domain and domain in ("general", "engineering", "office"):
            query = "SELECT id, domain, difficulty, text FROM sentences WHERE domain = ? ORDER BY RANDOM() LIMIT ?"
            params = (domain, limit)
        else:
            query = "SELECT id, domain, difficulty, text FROM sentences ORDER BY RANDOM() LIMIT ?"
            params = (limit,)
        async with db.execute(query, params) as cursor:
            rows = await cursor.fetchall()
            return [dict(row) for row in rows]
...
async def save_training_feedback(
    ...
    domain: str,
    tenant_id: str = "default",
    user_id: str = "training",
) -> int | None:
    """Save a training session feedback pair. Returns the new row id."""
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            """INSERT INTO correction_feedback
               (tenant_id, user_id, raw_whisper, model_output, user_action, user_edit, app_context, mode)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                tenant_id,
                user_id,
                transcribed_text,
                original_text,
                "edited" if corrected_text != transcribed_text else "approved",
                corrected_text if corrected_text != transcribed_text else None,
                f"training_sentence_id:{sentence_id}",
                domain,
            ),
        )
        await db.commit()
...
async def get_sentences_count() -> int:
    """Return total number of sentences in the DB."""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT COUNT(*) FROM sentences") as cursor:
            row = await cursor.fetchone()
            return row[0] if row else 0


async def get_user_stats(user_id: str | None = None, tenant_id: str = "default") -> dict:
    """Return personal usage stats for My Space.

    Returns total transcripts, total words, correction rate, and recognition score.
    Also returns weekly stats (last 7 days).
    """
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        ...
        if user_id:
            ...
            params_base = (tenant_id, user_id)
        else:
            user_filter = ""
            params_base = (tenant_id,)
        ...
        async with db.execute(
            f"""SELECT
                ...
                SUM(CASE WHEN corrected = 1 THEN 1 ELSE 0 END) AS total_corrected
               FROM transcriptions
               WHERE tenant_id = ? {user_filter}""",
            params_base,
        ) as cur:
            row = await cur.fetchone()
            total_transcripts = row["total_transcripts"] or 0
            total_words = int(row["total_words"] or 0)
            total_corrected = row["total_corrected"] or 0

        # Weekly stats (last 7 days)
        async with db.execute(
            f"""SELECT
                COUNT(*) AS week_transcripts,
                SUM(length(text) - length(replace(text, ' ', '')) + 1) AS week_words
               FROM transcriptions
               WHERE tenant_id = ? {user_filter}
            ...
            row = await cur.fetchone()
            week_transcripts = row["week_transcripts"] or 0
            week_words = int(row["week_words"] or 0)

    # Correction rate: how often raw != corrected (lower = better recognition)
    correction_rate = round(total_corrected / total_transcripts, 4) if total_transcripts > 0 else 0.0
    # Recognition score: 1 - correction_rate (higher = VoiceFlow understands you better)
    recognition_score = round(1.0 - correction_rate, 4)
    ...
    return {
        ...
        "week_transcripts": week_transcripts,
...
async def get_user_corrections(
    ...
    """Return last N transcriptions that had corrections (raw_text != text).

    Each row includes raw_text (what was said) and text (what was corrected to).
    """
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row

        if user_id:
            query = """SELECT id, raw_text, text, created_at, mode
                       FROM transcriptions
                       WHERE tenant_id = ? AND user_id = ?
                         AND corrected = 1 AND raw_text IS NOT NULL
