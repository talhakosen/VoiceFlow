...
`TranscriptionResult`, `StatusResponse`, `HistoryItem`, `HistoryResponse` → `BackendService.swift`'te (API contract ile birlikte).
...
enum AppSettings {
    ...
    static let userName           // Opsiyonel display name
    static let userDepartment     // Opsiyonel departman
}
```
